...
function M.apply_action(model, action)
	...
	elseif action.type == actions.ACTION_TYPE.TICK then
		...
		if self.target_uid then
			self.pick_animation_tick = self.pick_animation_tick + 1
			if self.pick_animation_tick > PICK_ANIMATION_TICKS then
				model.money = model.money + 1
				mm.delete_object(model, action.uid)
				return
			end
			local target_pos = model.objects[self.target_uid].position + vmath.vector3(0, 128, 0)
			local progress = self.pick_animation_tick / PICK_ANIMATION_TICKS
			self.position = self.init_position * (1 - progress) + target_pos * progress
		end
	end
end
...
function M.player_interact(uid, model, p_uid)
	...
	self.target_uid = p_uid
	self.pick_animation_tick = 0
	self.init_position = vmath.vector3(self.position)
end
